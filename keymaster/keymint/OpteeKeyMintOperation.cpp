...
#define TAG OpteeKeyMintOperation
...
OpteeKeyMintOperation::OpteeKeyMintOperation(shared_ptr<OpteeKeymaster> implementation,
                                               keymaster_operation_handle_t opHandle)
    : impl_(std::move(implementation)), opHandle_(opHandle) {}

OpteeKeyMintOperation::~OpteeKeyMintOperation() {
    if (opHandle_ != 0) {
        abort();
    }
}
...
ScopedAStatus OpteeKeyMintOperation::update(const vector<uint8_t>& input,
    ...
    if (serialized_size > OPTEE_KEYMASTER_SEND_BUF_SIZE) {
    ...
    const size_t max_chunk_size = OPTEE_KEYMASTER_SEND_BUF_SIZE - serialized_size;
    output->clear();
...
ScopedAStatus OpteeKeyMintOperation::finish(const optional<vector<uint8_t>>& input,      //
...
ScopedAStatus OpteeKeyMintOperation::abort() {
