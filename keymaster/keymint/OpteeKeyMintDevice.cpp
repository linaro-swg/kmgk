...
#include <optee_keymaster/OpteeKeyMintOperation.h>
...
ScopedAStatus OpteeKeyMintDevice::getHardwareInfo(KeyMintHardwareInfo* info) {
    info->versionNumber = 3;
    info->securityLevel = kSecurityLevel;
    info->keyMintName = "OpteeKeyMintDevice";
    info->keyMintAuthorName = "Google";
    info->timestampTokenRequired = false;
    return ScopedAStatus::ok();
}

ScopedAStatus OpteeKeyMintDevice::addRngEntropy(const vector<uint8_t>& data) {
...
ScopedAStatus OpteeKeyMintDevice::generateKey(const vector<KeyParameter>& keyParams,
...
ScopedAStatus OpteeKeyMintDevice::importKey(const vector<KeyParameter>& keyParams,
...
ScopedAStatus OpteeKeyMintDevice::upgradeKey(const vector<uint8_t>& keyBlobToUpgrade,
...
ScopedAStatus OpteeKeyMintDevice::deleteKey(const vector<uint8_t>& keyBlob) {
...
ScopedAStatus OpteeKeyMintDevice::deleteAllKeys() {
...
ScopedAStatus OpteeKeyMintDevice::destroyAttestationIds() {
    return kmError2ScopedAStatus(KM_ERROR_UNIMPLEMENTED);
}

ScopedAStatus OpteeKeyMintDevice::begin(KeyPurpose purpose, const vector<uint8_t>& keyBlob,
    ...
    result->operation = ndk::SharedRefBase::make<OpteeKeyMintOperation>(impl_, response.op_handle);
    return ScopedAStatus::ok();
}

ScopedAStatus OpteeKeyMintDevice::deviceLocked(
...
ScopedAStatus OpteeKeyMintDevice::earlyBootEnded() {
    keymaster::EarlyBootEndedResponse response = impl_->EarlyBootEnded();
    return kmError2ScopedAStatus(response.error);
}

ScopedAStatus OpteeKeyMintDevice::convertStorageKeyToEphemeral(
...
ScopedAStatus OpteeKeyMintDevice::getRootOfTrustChallenge(array<uint8_t, 16>* /* challenge */) {
    return kmError2ScopedAStatus(KM_ERROR_UNIMPLEMENTED);
}

ScopedAStatus OpteeKeyMintDevice::getRootOfTrust(const array<uint8_t, 16>& challenge,
...
ScopedAStatus OpteeKeyMintDevice::sendRootOfTrust(const vector<uint8_t>& /* rootOfTrust */) {
    return kmError2ScopedAStatus(KM_ERROR_UNIMPLEMENTED);
}
